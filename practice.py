...
def my_padding(img, shape, boundary=0):
    ...
    res = np.zeros((row + (2 * pad_sizeY), col + (2 * pad_sizeX)), dtype=np.float64)
...
gaus1D = my_getGKernel((5, 5), 3)

# 1D filtering
img1D = my_filtering(g, gaus1D)
img1D = my_filtering(img1D, gaus1D.T)

dst = cv2.GaussianBlur(g, (255, 255), 1)
